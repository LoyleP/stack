...
struct DeckView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                if showList {
                    listDrawerOverlay(geometry: geometry)
                }
            }
        }
    ...
    private var headerLayer: some View {
        VStack {
            HStack {
                Button(action: {
                    withAnimation(Orbit.Dynamics.panel) { showList = true }
                }) {
    ...
    private var controlLayer: some View {
        VStack {
            Spacer()
            if showInput {
                ControlBar(text: $newOptionText, focusState: $isInputFocused, onAdd: {
                    viewModel.addOption(newOptionText)
                    newOptionText = ""
                }, onShuffle: viewModel.performShortShuffle)
            ...
            DragGesture(minimumDistance: 0)
                .onChanged { _ in
                    if !viewModel.isShuffling {
                        lastPressTime = Date()
                        withAnimation(Orbit.Dynamics.gesture) { isHolding = true }
                        viewModel.startContinuousShuffle()
                    }
                }
                .onEnded { _ in
                    withAnimation(Orbit.Dynamics.gesture) { isHolding = false }
                    if Date().timeIntervalSince(lastPressTime) < 0.2 {
                        withAnimation(Orbit.Dynamics.physics) { isTapping = true }
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                            isTapping = false
                            viewModel.endContinuousShuffle()
                        }
                    } else {
                        viewModel.endContinuousShuffle()
                    }
    ...
    @ViewBuilder
    private func listDrawerOverlay(geometry: GeometryProxy) -> some View {
        Group {
    ...
    // MARK: - Gesture & Math Helpers

    private var dragGesture: some Gesture {
        DragGesture()
            .onChanged { v in
                withAnimation(Orbit.Dynamics.gesture) { dragOffset = v.translation }
            }
            .onEnded { v in
                if abs(v.translation.width) > 140 {
                    ...
                    withAnimation(Orbit.Dynamics.physics) { dragOffset = .zero }
                }
            }
    }
    ...
    func getOffset(for item: Item) -> CGSize {
        ...
        var y: CGFloat = 0
        if item.id == viewModel.items.last?.id && !viewModel.isShuffling {
            x += dragOffset.width
            y += dragOffset.height
        }
    ...
    func getRotation(for item: Item) -> Angle {
        ...
        if let x = viewModel.cardOffsets[item.id] {
            rot += Double(x / 18)
        }
        return .degrees(rot)
    }
    ...
    func getScale(for item: Item) -> CGFloat {
        ...
        if item.id == viewModel.items.last?.id && dragOffset != .zero && !viewModel.isShuffling {
