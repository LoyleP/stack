...
struct DeckView: View {
            ...
            if items.isEmpty {
                ...
                ForEach(items) { item in
                    CardView(text: item.text, color: item.color)
                        ...
                        .gesture(
                            item.id == items.last?.id && !isShuffling
                                ? dragGesture : nil
                        )
                }
            }
    ...
    private var emptyStateView: some View {
        VStack(spacing: 16) {
            Image(systemName: "square.stack.3d.up.slash").font(
                .system(size: 60)
            ).foregroundStyle(.white.opacity(0.4))
            Text("No options left.").font(Orbit.headingFont()).foregroundStyle(
                .white.opacity(0.4)
            )
        }
    }
    ...
    func getScale(for item: Item) -> CGFloat {
        ...
        if item.id == items.last?.id && dragOffset != .zero && !isShuffling {
            ...
            .onEnded { value in
                ...
                if abs(value.translation.width) > threshold {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                        // UPDATED: Use semantic haptic for manual card cycle
                        Haptics.shared.shuffleTuck()
        ...
        VStack {
            ...
            if showInput {
                ControlBar(
                    ...
                    onAdd: addOption,
                    onShuffle: performShortShuffle
                )
                .padding(.bottom, 10).padding(.horizontal, 16).transition(
                ...
                HStack(spacing: 16) {
                    ...
                    ZStack {
                        Circle().fill(.white).frame(width: 56, height: 56)
                            ...
                            .opacity(isTapping ? 1 : 0).scaleEffect(
                                isTapping ? 1 : 0.7
                            )
                        ...
                        .padding(.horizontal, 30).frame(height: 56).background(
                            Capsule().fill(.white)
                        )
                        .opacity(isTapping ? 0 : 1).scaleEffect(
                            isTapping ? 0.8 : 1
                        )
                    }
    ...
    func addOption() {
        ...
        if items.contains(where: {
            $0.text.lowercased() == newOptionText.lowercased()
        }) {
            newOptionText = ""
            return
        }
