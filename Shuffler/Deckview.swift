...
struct DeckView: View {
            ...
            ZStack {
                ...
                if showList {
                    Color.black.opacity(0.4)
                        ...
                        .transition(.opacity)
                        .onTapGesture {
                            withAnimation(Orbit.Dynamics.panel) { showList = false }
                        }
                    
                    VStack {
                        Spacer()
                        OptionsListView(items: $items, isPresented: $showList)
                            .frame(height: geometry.size.height * 0.7)
                            .clipShape(RoundedRectangle(cornerRadius: 32, style: .continuous))
                            .overlay(RoundedRectangle(cornerRadius: 32, style: .continuous).stroke(Orbit.glassBorder, lineWidth: Orbit.glassBorderWidth))
                    }
    ...
    private var backgroundLayer: some View {
        LinearGradient(
            colors: [items.last?.color.opacity(0.35) ?? .gray.opacity(0.1), Color(red: 0.05, green: 0.06, blue: 0.07)],
            startPoint: .topLeading, endPoint: .bottomTrailing
        )
        .ignoresSafeArea()
        .animation(Orbit.Dynamics.background, value: items.last?.id)
        .onTapGesture { if showInput { isInputFocused = false; hideKeyboard(); withAnimation(Orbit.Dynamics.panel) { showInput = false } } }
    }
    ...
    private var cardLayer: some View {
        ...
        .animation(Orbit.Dynamics.physics, value: items.map { $0.id })
    }
    ...
    func getOffset(for item: Item) -> CGSize {
        ...
        var y: CGFloat = 0
        if item.id == items.last?.id && !isShuffling { x += dragOffset.width; y += dragOffset.height }
        return CGSize(width: x, height: y)
    }

    func getRotation(for item: Item) -> Angle {
        var rot = item.rotation
        if item.id == items.last?.id && !isShuffling { rot += Double(dragOffset.width / 15) }
        if let x = cardOffsets[item.id] { rot += Double(x / 18) }
        return .degrees(rot)
    }
    ...
    func startContinuousShuffle() {
        ...
        withAnimation(Orbit.Dynamics.gesture) { dragOffset = .zero }
        ...
        shuffleTask = Task { @MainActor in
            while !Task.isCancelled {
                ...
                withAnimation(Orbit.Dynamics.element) { activeShuffleId = idToMove; cardOffsets[idToMove] = 450 }
                try? await Task.sleep(nanoseconds: 120_000_000)
                Haptics.shared.shuffleTuck()
                if let index = items.firstIndex(where: { $0.id == idToMove }) { let card = items.remove(at: index); items.insert(card, at: 0) }
    ...
    func endContinuousShuffle() {
        ...
        withAnimation(Orbit.Dynamics.physics) { items = remaining + [winner]; isShuffling = false; isTapping = false; cardOffsets.removeAll(); activeShuffleId = nil }
        ...
        DragGesture().onChanged { v in withAnimation(Orbit.Dynamics.gesture) { dragOffset = v.translation } }
            .onEnded { v in
                if abs(v.translation.width) > 140 {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { Haptics.shared.shuffleTuck(); let card = items.removeLast(); items.insert(card, at: 0); dragOffset = .zero }
    ...
    private var controlLayer: some View {
        VStack {
            ...
            if showInput {
                ...
                HStack(spacing: 16) {
                    Button(action: { withAnimation(Orbit.Dynamics.element) { usedSessionColors.removeAll(); showInput = true; isInputFocused = true } }) { Image(systemName: "plus") }.buttonStyle(.orbitGlass)
                    ZStack {
                    ...
                    .scaleEffect(isHolding ? 1.1 : 1.0).animation(Orbit.Dynamics.element, value: isTapping).animation(Orbit.Dynamics.gesture, value: isHolding)
                    .contentShape(Capsule()).gesture(DragGesture(minimumDistance: 0).onChanged { _ in if !isShuffling { lastPressTime = Date(); withAnimation(Orbit.Dynamics.gesture) { isHolding = true }; startContinuousShuffle() } }
                        .onEnded { _ in withAnimation(Orbit.Dynamics.gesture) { isHolding = false }; if Date().timeIntervalSince(lastPressTime) < 0.2 { withAnimation(Orbit.Dynamics.physics) { isTapping = true }; DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) { endContinuousShuffle() } } else { endContinuousShuffle() } })
    ...
    private var headerLayer: some View {
        VStack {
            HStack { Button(action: { withAnimation(Orbit.Dynamics.panel) { showList = true } }) { Image(systemName: "list.bullet") }.buttonStyle(.orbitGlass); Spacer() }
            .padding(.horizontal, 20).padding(.top, 8); Spacer()
        }
    }

    func performShortShuffle() { if !isShuffling { withAnimation(Orbit.Dynamics.physics) { isTapping = true }; startContinuousShuffle(); DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) { endContinuousShuffle() } } }
    func addOption() {
        ...
        withAnimation(Orbit.Dynamics.element) { items.append(Item(text: newOptionText, colorName: picked)) }
